...
DEFAULT_FONTS = [os.path.join(DATA_DIR, 'ArialBold.ttf')]
# DEFAULT_FONTS = ['ArialBold.ttf']
...
if __name__ == "__main__":
    cap = ImageCaptcha(160, 60)
    import string
    import numpy as np
    chars = random.sample(string.digits + string.ascii_letters, 4)
    cap1 = cap.generate(chars)
    text = "".join(chars)
    # cap.write(chars, text + ".jpg")
    im = Image.open(cap1)
    # im = convert2gray(np.array(im))
